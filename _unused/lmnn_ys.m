function [M, L, Y, C] = lmnn_ys(X, labels, my_mu, my_no_targets, my_eta)
%LMNN Learns a metric using large-margin nearest neighbor metric learning
%
%   [M, L, Y, C] = lmnn_ys(X, labels)
%
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    eta = my_eta;               % learning rate
    %eta = .1;               % learning rate
    mu = my_mu;                % weighting of pull and push terms
    %mu = .5;                % weighting of pull and push terms
    tol = 1e-3;             % tolerance for convergence
    best_C = Inf;           % best error obtained so far
    best_M = M;             % best metric found so far
    no_targets = my_no_targets;         % number of target neighbors
