...
#
            #CLIBS="$CLIBS -lstdc++"
            CLIBS="$CLIBS -lstdc++ -I/usr/local/lib -lgomp" 
            # C++keyName: Clang++
            ...
            CXX='xcrun /usr/local/bin/g++-5'
            #CXXFLAGS="-fno-common -fexceptions -arch $ARCHS -isysroot $MW_SDKROOT -mmacosx-version-min=$MACOSX_DEPLOYMENT_TARGET"
            CXXFLAGS="-std=c++11 -fno-common -fexceptions -arch $ARCHS -isysroot $MW_SDKROOT -mmacosx-version-min=$MACOSX_DEPLOYMENT_TARGET"
            #CXXLIBS="$MLIBS -lstdc++"
