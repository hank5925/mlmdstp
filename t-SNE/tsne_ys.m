function ydata = tsne_ys(X, labels, no_dims, initial_dims, perplexity, my_epsilon)
%TSNE Performs symmetric t-SNE on dataset X
%
%   mappedX = tsne_ys(X, labels, no_dims, initial_dims, perplexity)
%   mappedX = tsne_ys(X, labels, initial_solution, perplexity)
%
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if ~exist('my_epsilon', 'var') || isempty(my_epsilon)
        my_epsilon = 500;
    end
    ...
    % Run t-SNE
    if initial_solution
        ydata = tsne_p_ys(P, labels, ydata, my_epsilon);
    else
        ydata = tsne_p_ys(P, labels, no_dims, my_epsilon);
    end
