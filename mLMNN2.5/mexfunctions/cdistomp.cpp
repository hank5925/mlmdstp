                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  sx2=(double*)mxCalloc(n,sizeof(double));

  
  /* compute sx2 */
  for (i=0;i<n;i++)
   sx2[i]=dotp(m,&X[i*m],&X[i*m]);

#pragma omp parallel shared(dis,sx2,X) private(i1,i2)
  {
#pragma omp for
  for (i=0;i<inds;i++){
    i1=(int) a[i]-1;
    i2=(int) b[i]-1;
    dis[i]=sx2[i1]+sx2[i2]-2*dotp(m, &X[i1*m],&X[i2*m]);
  }
  }
}
