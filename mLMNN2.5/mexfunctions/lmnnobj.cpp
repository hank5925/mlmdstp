...
double computeloss(double *X, short int *T, short int *I, int d, int kt, int ki,int i,double *grad){
    ...
    // compute distances to target neighbors
     for(int k=0; k<kt; k++) {
         //FIXME: Ying-Shu set those target neighbors of unlabeled data as -1,
         //       so be careful!!!
         //       I mean T[idx] might be -1!!!
         if (T[k] == -1) {
             break;  // don't need to do the rest...
             //fprintf(stderr, "Should never see this line\n");
         }
    ...
    for(int k=0; k<ki; k++) {
         //FIXME: Ying-Shu set those potential impostors of unlabeled data as -1,
         //       so be careful!!!
         //       I mean I[idx] might be -1!!!
         if (I[k] == -1) break;  // don't need to do the rest...
