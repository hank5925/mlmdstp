...
%%%% TODO: Ying-Shu gen effect
dfG=vec(SOD(x,gen(1,:),gen(2,:)));


if(pars.scale)
 Lx=L*x;
%%%% TODO: Ying-Shu NN effect, replace those point having NN = -1
%%%%       with the original data point.
%%%%       Don't let them effect the sqrt mean calculation.
 NNfix = NN(end,:);
 NNfix(find(NNfix==-1)) = find(NNfix==-1);
 sc=sqrt(mean(sum(((Lx-Lx(:,NNfix))).^2,1)));
...
for iter=1:pars.maxiter
 ...
 Ni=zeros(Kg,N);
 for nnid=1:Kg
%%%% TODO: Ying-Shu NN effect, NN's -1 part won't be used anyway
%%%%       replace those point having NN = -1 with the original
%%%%       data point. Don't let them effect the sqrt mean calculation.
  NNfix2 = NN(nnid,:);
  NNfix2(find(NNfix2==-1)) = find(NNfix2==-1);
  %Ni(nnid,:)=(sum((Lx-Lx(:,NN(nnid,:))).^2,1)+1);
  Ni(nnid,:)=(sum((Lx-Lx(:,NNfix2)).^2,1)+1);
 end;
...
%%%% TODO: Ying-Shu NN effect, effect decided by imp
...
%%%% TODO: Ying-Shu NN effect, effect deciede by imp
...
gen=[gen1;gen2];
%%%% TODO: Ying-Shu delete those data with -1 target neighbor.
%%%%       In the following step, only dfG is going to use gen,
%%%%       Which is the pulling term |L(xi) - L(xj)|^2, so deleting
%%%%       them should be fine.
gen = gen(:,find(gen(1,:)~=-1));
...
L=L(find(mL),:);
Lx=L*x;
%%%% TODO: Ying-Shu NN effect, but we don't use mtree.
...
imp=[];
%%%% TODO: Ying-Shu take care of impostors, unlabeled data are neither
%%%%       in the list of impostors, nor in the list of data point to
%%%%       search for impostors.
start = 1;
if (un(1) == 0)
    start = 2;
end;
for c=start:length(un)-1
if(~pars.quiet)fprintf('All impostors for class %i    \r',c);end;
 for c2=c+start:length(un)
     try
...
Lx=L*x;
%%%% TODO: Ying-Shu NN effect
%%%%       I know it's silly to change back to 1, but the point is
%%%%       I don't want to make anyone use NN without knowing that
%%%%       there are some unlabeled data point should be used in caution.
%%%%       The thing is, limps is not going to use those unlabeled data
%%%%       anyway since I have passed through it.
NN(find(NN==-1))=1;
Ni=sum((Lx-Lx(:,NN)).^2,1)+2;
...
%%%% TODO: Ying-Shu don't add any impostors for unlabeled data
%%%%       This also takes care of label - unlabel pair since it uses
%%%%       index = find(y>c) as the scope of possible impostors
%%%%       which will never include c==0 case.
 if c==0
  continue;
 end;
 i=find(y==c);
